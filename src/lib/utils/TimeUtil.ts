module TimeUtil {
    ...
    export function toChineseStr(time:number, posfix:string=""):string {
        let result = "";
        if (time >= 86400) {
            result = Math.ceil(time / 86400) + "天" + posfix;
        } else if (time > 3600)
            result = Math.ceil(time / 3600) + "小时" + posfix;
        else {
            result = Math.ceil(time / 60) + "分钟" + posfix;
        }
        return result;
    }
}