...
self.onmessage = function(event) {
    ...
    if (action == "loadJson") {
        loadJson(param);
    } else if (action == "loadGameConfig") {
        loadConfigZip(param);
    } else if (action == "loadSkeFile") {
        logSkeFile(param);
    }
};

function loadConfigZip(url) {
    ...
    xhr.onload = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            for (var i = 0; i < keys.length; i++) {
                var key = keys[i];
                var arr = key.split("_");
                result[arr[0]] = zip.readObj(key);
            }
            postMessage({url:url, content:result});
        }
    };
    xhr.send();
}
...
function logSkeFile(url) {
    ...
    xhr.onload = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            var name = "";
            var data = null;
            if (fileName.indexOf("_bin") >= 0) {
                name = fileName.replace("_bin", "bone");
                name += "_ske.dbbin";
                data = zip.readBytes(name);
            } else {
                name = fileName + "_ske.json";
                data = zip.readObj(name);
            }
            postMessage({url:name, content:data});
        }
    };
    xhr.send();
}
...
ZipFile.prototype = {
    ...
    readObj:function(filename) {
            var out = {};
            var int8Array = this.readBytes(filename);
            if (int8Array) {
                var str = this.utf8ArrayToStr(int8Array);
                out = JSON.parse(str);
            }
            return out;
    },
